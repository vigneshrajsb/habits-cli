...
async function autoTursoSetup(config: Config) {
  ...
  if (tursoDbExists("habits")) {
    const urlResult = exec("turso", ["db", "show", "habits", "--url"]);
    dbUrl = urlResult.stdout;
    s.stop(`Using existing database: ${dbUrl}`);
  } else {
    const create = exec("turso", ["db", "create", "habits"]);
    if (!create.ok) {
  ...
  const tokenResult = exec("turso", ["db", "tokens", "create", "habits"]);
  if (!tokenResult.ok) {
...
async function manualTursoSetup(config: Config) {
  ...
  const url = await text({
    ...
    placeholder: "libsql://habits-username.turso.io",
    validate: (value) => {
